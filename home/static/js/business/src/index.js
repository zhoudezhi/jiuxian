...
/*	$(".mainBanner").slide({
	...
	$(".timeLimBox .titieBox li").click(function(e) {
		...
		$parents.find(".titleSlider").stop().animate({"left":(width+1)*index},500);
	...
	$(".numBuy .collect-but").one('click',function(){
		...
		$(this).find('em').show();
		$(this).find('em').stop().animate({'top':'-20px'},200);
		$(this).find('em').stop().animate({'top':'-40px','opacity':'0'},500,function(){
			$(this).hide();
		});
	});
	/*限时抢购滚动*/
	...
	var  creatDIV=function(index){
		...
		$(".bottle_"+index).css({"left":p.left,"top":p.top});
	};
	...
	$('.fixDiv div').each(function(i, element) {
        ...
        $(this).bind('click',function(){
			...
			$('html,body').stop().animate({'scrollTop':arrPosition[i]},400);
		});
    });
